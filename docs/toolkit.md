...
すべてこのパッケージ向けの新規記述です。基本の6ツールはPython標準ライブラリだけを使用し、実測アニメーションの追加ツールはPillowとimageio-ffmpeg、チェックポイント比較図の追加ツールはmatplotlibを使います。第三者のソースや私的な実験フォルダから実装をコピーしていません。
...
| `render_motion_history.py` | 選択した実測XY軌跡・向き・足上げの図をMP4/GIFに描画 | 物理シミュレーション、メーカー形状、未知の運動の生成 |
| `render_checkpoint_comparison.py` | 公開済みチェックポイント集計JSONから位置ずれの比較図を描画 | 原配列の再計算、録画生成、単一指標による技能合格の判定 |

## 第32回の比較図

`evidence/l27-evaluation.json` の親・100・200・250更新の数値を使用します。縦軸の範囲は動作ごとに異なり、早期終了の記録時間も表示します。

```bash
python tools/render_checkpoint_comparison.py
```

図ごとの入力・描画コード・出力SHAは `evidence/media-publication.json` に記録しています。

## READMEのアニメーション

入力は `evidence/forward-motion.json` の選択した自作測定値です。環境の生ログやモデルを読みません。日本語を表示できるフォントを利用者側で指定します。フォントファイルは同梱しません。

```bash
python tools/render_motion_history.py --input evidence/forward-motion.json --output-dir output-media --font FONT_FILE
```

MP4は全600サンプルを50fpsで表示。GIFは原則5サンプルごとに縮小し、最後の記録点も含め、フレームの表示時間を調整して全体の12秒を維持します。軌道の補間・平滑化・成功区間の選択はしません。元の録画そのものではなく図による再描画なので、接触形状や自己干渉の検証には使えません。

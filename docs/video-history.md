...
最初にユーザーが指定したCAD画像1点と録画5本を、既存プロジェクトの原本とSHAで照合して掲載しました。その後の結果動画を含め、現在は実録画・比較動画21本を公開しています。CAD画像は1点、実測チャートの再描画MP4は別枠です。ファイル名だけで実験を推測していません。[READMEの動画一覧](../README.md#映像でたどる開発履歴)から、元MP4と全区間のGIFプレビューを開けます。
...
これらは開発順を示す参考履歴で、モデル・指令・物理設定が異なります。旧2197・旧250を現在の直近19回へ混ぜず、H04・L00を二重に数えません。B0とCAD画像も改善回数へ加算しません。2197・250・599・249は保存番号で、改善ループ回数とは別です。全開発の通算回数は引き続き未確定です。
...
L13とL14のMP4を1本ずつ追加して、実録画・比較動画は19本から21本になりました。CAD画像1点と再描画チャートは別枠です。GIF、動画の公開、同じ学習の途中モデル評価を追加ループには数えず、直近系列は19回のままです。当面の新規学習は500更新の方針ですが、新しい学習・ジャンプ・後ろ足2本立ちの結果を掲載したものではありません。

...
最初にユーザーが指定したCAD画像1点と録画5本を、既存プロジェクトの原本とSHAで照合して掲載しました。その後の結果動画を含め、現在は実録画・比較動画19本を公開しています。ファイル名だけで実験を推測していません。[READMEの動画一覧](../README.md#映像でたどる開発履歴)から、元MP4と全区間のGIFプレビューを開けます。
...
これらは開発順を示す参考履歴で、モデル・指令・物理設定が異なります。旧2197・旧250を現在の直近17回へ混ぜず、H04・L00を二重に数えません。B0とCAD画像も改善回数へ加算しません。2197・250・599・249は保存番号で、改善ループ回数とは別です。全開発の通算回数は引き続き未確定です。
...
各600フレーム・50fps・12秒の比較枠です。前進原本は557フレーム・11.14秒で終了し、残り43フレームは「以後の映像なし」。終端原本画像は自動リセット後描画の可能性があり、失敗判定はリセット前記録を使っています。GIFは終端557・最初の黒558を含め元の時刻で表示します。追加2本で実録画・比較動画は19本、完了した有限試行は17回です。

...
最初に指定されたCAD画像1点と録画5本から、選択した各回の結果・比較を追加してきました。今回、第31回A/B/Cの14秒比較MP4とGIFを各3点追加し、公開実録画・比較MP4は44→47本となりました。GIFはMP4本数へ加算しません。最新は第31回ABC、第30回ABCと第27回A/Bも履歴に保持します。CAD画像1点と実測チャート再描画MP4は別枠です。[README](../README.md#映像でたどる開発履歴)の最初・よちよち歩き・第5・10・15・20・25回のダイジェスト7件も維持します。14秒比較は表示用の抜粋で、各欄の全評価時間とは区別します。
...
これらは開発順を示す参考履歴で、モデル・指令・物理設定が異なります。旧2197・旧250を現在の直近31回へ混ぜず、H04・L00を二重に数えません。B0とCAD画像も改善回数へ加算しません。2197・250・599・249は保存番号で、改善ループ回数とは別です。全開発の通算回数は引き続き未確定です。

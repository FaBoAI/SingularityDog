...
最初にユーザーが指定したCAD画像1点と録画5本を、既存プロジェクトの原本とSHAで照合して掲載しました。第30回A/Bを先行公開し、今回Cの14秒比較MP4とGIFを各1点追加しました。第30回A/B/Cの比較がそろい、公開実録画・比較MP4は43本から44本になりました。GIFはMP4本数へ加算しません。 最新の掲載対象は第30回A/B/Cで、第27回A/Bの動画も履歴に保持します。CAD画像1点と実測チャート再描画MP4は別枠です。[README](../README.md#映像でたどる開発履歴)の最初・よちよち歩き・第5・10・15・20・25回のダイジェスト7件も保持します。14秒比較は表示用の抜粋で、各欄の全評価時間とは区別します。
...
先行した数値公開時点では直近系列30回・公開実録画41本でした。以下の動画追加は新しい試行に数えず、第31回の学習は未開始です。

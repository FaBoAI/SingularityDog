...
2026-09-18更新。数値結果は[第37回](l32-evaluation.md)まで公開しています。新しい録画の掲載は第31回A/B/Cまでで、全47本の公開録画・比較MP4を保持しています。第32〜37回の動画を過去動画で代用していません。

[READMEの動画で見る進捗](../README.md#動画で見る進捗)は第1・2・3・10・20・30回です。第1・2回は記録のみ、第3回は実録画ではない[初期の計測比較チャート](media/forward-history.mp4)、第10・20・30回はシミュレーション録画を掲載します。[別の節目一覧](milestone-history.md)には第40回を未実施として残しています。最初に掲載した旧保存番号2197の映像は、第1回の映像とは別です。下の記録は各時点の掲載履歴です。

最初に指定されたCAD画像1点と録画5本から、選択した各回の結果・比較を追加してきました。第31回A/B/Cの14秒比較MP4とGIFを各3点追加した際に、公開実録画・比較MP4は44→47本となりました。GIFはMP4本数へ加算しません。最新は第31回ABC、第30回ABCと第27回A/Bも履歴に保持します。CAD画像は初期版と黄色版の2点で、実測チャート再描画MP4とともに録画本数とは別枠です。以前のダイジェストに掲載した最初・よちよち歩き・第5・10・15・20・25回のメディアは、この動画履歴に保持します。14秒比較は表示用の抜粋で、各欄の全評価時間とは区別します。

第32回は[評価集計と比較図](l27-evaluation.md)を公開しました。動画は未回収・未掲載で、最新の公開動画は第31回のままです。第32回掲載時点の32試行と録画・比較MP4の47本、実測チャート再描画MP4の1本を別集計します。
...
| `assembly_preview.png` | 黄色・黒の全体CAD更新版（2026-09-14）<br>[README先頭のCAD画像](media/cad-yellow-assembly-20260914.png) | 脚・取手・上部を黄色、Body・顔を黒へ。B02/B04/B06/B08のcandidate_v3補強を反映。以降の共通底板・顔固定・左右連結部r2・足先r3は別部品改訂で、この全体図への統合は示さない |
...
これらは開発順を示す参考履歴で、モデル・指令・物理設定が異なります。旧2197・旧250を直近系列の実験回数へ混ぜず、H04・L00を二重に数えません。B0とCAD画像も改善回数へ加算しません。2197・250・599・249は保存番号で、改善ループ回数とは別です。全開発の通算回数は引き続き未確定です。
...
第30回の伏せはB/Cで2回とも連続2秒保持と四足復帰を達成しましたが、Cの最大XY57.52mm・方位ずれ21.38°が残りました。挨拶は足先150mm以上・4往復・四足復帰を維持し、最大XYはCで63.02mmでした。先行した数値公開ではMP4/GIFを追加せず、その時点で動画は別途回収・検査中でした。数値の全評価区間と14秒の表示用ダイジェストは別です。

先行した数値公開時点では直近系列30回・公開実録画41本でした。以下の第30回動画追加は新しい試行に数えません。その数値公開時点では第31回の学習は未開始でした。


## L25・第30回：A/B/Cの14秒3×3比較
...
各条件を別々の3×3比較として並べます。6方向移動は各12秒の全区間、伏せは24秒評価の第1周期12秒を表示します。挨拶は18秒周期、お座りは16秒周期の先頭14秒までで、両者の第1周期は途中までです。12秒で表示区間が終わる欄は、その区間の最後のフレームをカラーで静止表示し、終了時刻とSTILLを示します。この静止時間は実動作や保持成功へ加算しません。

各プレビュー画像内の左上、元の黒いcaption欄より下の背景に、黒い縁取りの明るいシアンで英語動作名を重ねます。英語名はForward、Backward、Move Left、Move Right、Turn Left、Turn Right、Lie Down、Wave、Sitです。

FULL EVALの合否と本文の測定値は、各原評価の全区間（移動12秒、伏せ24秒、挨拶36秒、お座り32秒）に基づきます。14秒の表示から2周期の成立や全区間の最大値を再判定しません。各条件7/9合格、伏せ・挨拶の総合未達、1500主更新と4専門方策の区別は不変です。単一方策での連続切替や、実機での成功を示す動画ではありません。


## L26・第31回：ABCの14秒比較と途中終了
...
各条件500主更新、計1500主更新後の9動作を別々の3×3で表示します。全条件7/9合格で、新たな合格動作は0。MP4は960×800・50fps・700フレーム・14秒、GIFは対応する14秒全区間の縮小表示です。原録画と編集物を区別し、原本SHA・全復号・表示確認は個別台帳へ登録します。[選択した評価結果](l26-evaluation.md)。

移動6方向は12秒、A伏せは第1周期12秒、B/C伏せは実終端9.68/9.74秒までです。表示終了後はその最後のカラー画像を保持し、STILL/TERMINATEDを示します。挨拶とお座りは18/16秒周期の先頭14秒で途中まで。英語名は各シミュレーション画像左上の背景へシアンで表示します。

Cの挨拶は23.18/36秒で禁止接触終了し、14秒抜粋には実終端が映りません。そのため映像表示中も全記録の途中終了を明記します。FULL EVALの合否は元評価によるものです。終端描画に自動リセット後の姿勢・空の背景があっても、安定復帰・成功姿勢の証明とはせず、カラー静止時間も保持成功へ加算しません。

原27動画は検証しましたが、今回の公開対象は表示用ABC比較MP4/GIFの6ファイルです。原計測配列・モデル重み・検査用静止画は追加しません。既存44本と履歴7件を保持し、実験系列は31回、録画・比較MP4は47本として別集計します。

...
![第25回・9動作の全36秒を3×3で比較](media/l20-nine-motions.gif)

[9分割MP4・36秒・50fps](media/l20-nine-motions.mp4) · [判定・同D16親との比較](l20-evaluation.md)
...
個別9本と比較1本の計10本を追加し、実録画・比較動画は39本になりました。実測チャートの再描画MP4は別枠の1本です。全原動画を同時開始・等速で並べ、終了後は時刻付きの黒いパネルへ切り替えます。4方策を同時実行した一つの機体や、同一エピソード内の連続切替を示す映像ではありません。各評価の最初の黒いフレームや終了時の描画も残し、停止前の実測記録で判定しました。

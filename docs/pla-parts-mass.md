...
現在の印刷候補は[STL部材一覧](stl-parts.md)の89個です。このページは過去の部品構成の換算質量を残した資料で、最新構成や実機総重量ではありません。[BOM](bom.md)で購入品の実測・仮値を区別しています。

この表は**L13/L14時点の構成を保存した記録**です。第21回L16で改訂した中央ケースや新しい機器固定具を含む最新PLA総量として使わないでください。第21回の全機体のシミュレーション質量は約8.19654kgで、PLAだけの重量とは異なります。[第21回の結果](l16-evaluation.md)。

...
**第35回までのGPU学習結果を整理しました。** 第33回1,000、第34回400、第35回277の計1,677主更新を実施。API動作確認30更新は別枠です。第33・34回では移動6方向とお座りの7/9合格を維持しましたが、伏せ・挨拶の新しい合格モデルは得られず、置き換えていません。

| 第35回の最新候補 | 実施した改善と残課題 |
|---|---|
| 伏せ・38更新 | 全256環境で2周期を学習。最終24秒評価はXY100.320mm・yaw5.120°・第2保持1.9725秒で未達 |
| 挨拶・57更新 | 探索範囲を支持9関節へ限定し、36秒到達が1/256→194/256環境へ増加。最終36秒評価は挙上・往復・復帰を維持したが、XY61.583mmで50mm基準を超過 |

![第35回の最新保存モデル比較。測定値から描いた図で、録画ではありません。](docs/media/l30-checkpoint-comparison.png)

第35回は9件の有限試行で、完了6件・事前条件による停止2件・制御不具合による停止1件。最後の評価は2026-09-15 19:35:46 JSTに終了しました。各候補の更新数を合算した単一モデルや、9動作すべての再評価結果ではありません。第33〜35回の新規保存モデル35評価は全て総合未合格です。

[第35回の結果・残課題・第36回への修正方針](docs/l30-evaluation.md) · [第34回](docs/l29-evaluation.md) · [第33回](docs/l28-evaluation.md) · [第32回](docs/l27-evaluation.md) · [現在地](docs/current-status.md)

第36回は9月16日いっぱいのGPU利用許可を受けて準備中です。位置ずれを減らす補正方向の診断、2周期の保持、実際の関節指令への反映を確認して進めます。最近のPLA補強・共通底板は、上記の固定D17による学習結果に含みません。最新の公開動画は下の第31回A/B/Cです。

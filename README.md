...
**確認できた直近の改善系列は21回です。** H01〜H04とL00〜L16を数え、診断・失敗を含みます。成功21回や全開発の通算回数ではありません。

**第21回・L16は、新しい機器配置のモデルで500更新（最終iteration499）を完了しました。** 学習終了は09/10 22:38、配色評価終了は22:45 JST。伏せ2秒保持と四足への復帰2秒保持を1回達成しましたが、ジャンプは未達です。40秒予定の評価は24.2秒で右前脚の太もも部分の禁止接触により停止しました。

最大方位ずれ21.26°、最大位置ずれ10.59cmで、位置保持と2周期完遂も未達。評価中の関節速度超過は0標本、学習探索中は25関節標本でした。接触相手は未特定で、接地・自己接触の区別は追加確認が必要です。[第21回の結果・課題](docs/l16-evaluation.md) · [指定色の評価動画・24.2秒](docs/media/l16-model499-prone-jump.mp4) · [第20回の記録](docs/l15-evaluation.md)

第18回・L13では1000更新後、6方向が既存の12秒開発基準を通過し、前進40.2521cm/sを達成しました。第19回・L14の指定model500は501更新後の別モデルです。過去の回数と結果は各履歴に残しています。

**当面の新規学習は500更新。** 第21回では中央バッテリーと下面セパレーター、機器固定具を新モデルへ反映しました。底板は白、脚・取手・固定具・顔枠は黄色、カーボン・QDD・足先球は黒、IMUは赤、顔は黒地に白い目です。配色評価では全660描画部品の材質と、変更前後の形状・物理属性の不変を確認しています。実機の組付け・PLA強度と、Fusionの最終色版アーカイブ保存は未完了です。[仕様の反映状況と次の課題](docs/next-loop-spec.md) · [L13/L14時点のPLA79個の重量表](docs/pla-parts-mass.md)。重量表は旧構成のCAD中実換算で、新機器固定具を含む最新構成の実印刷重量ではありません。第22回は前足1本を振る挨拶モーションの複数試行を計画中で、まだ実施回数へ加算しません。後ろ足2本立ちは未学習です。
...
READMEでは**最初・よちよち歩き・第5・10・15・20回の動画ダイジェスト**を掲載します。全ての録画・比較動画23本とCAD画像1点は引き続き公開し、[全動画の履歴](docs/video-history.md)と[21回の実行・評価履歴](docs/improvement-loops.md)からたどれます。改善回数21回と動画本数23本は別の集計です。
...
|---|---|---|
| **最初**<br>旧形状・保存番号2197<br>直近21回より前 | <img src="docs/media/archive-joystick2197-left.gif" width="480" alt="最初の掲載動画・旧形状での左移動。保存番号2197は改善回数ではありません"><br>[元MP4・5.98秒](docs/media/archive-joystick2197-left.mp4) | 外装付きの旧モデルで左移動を試行。同じ方策の別評価では速度・姿勢条件を満たしたが、後の足別診断では15mm以上の完了足上げは全脚0回。すり足が課題。 |
...
「最初」は以前の一覧の先頭にあった旧形状の動画です。保存番号2197は改善回数ではなく、直近21回へ加算しません。「よちよち歩き」は既存の第4回H04で、新しい実験回数ではありません。第20回のプレビューは24秒全区間で、終端の自動リセットも含みます。間の回や初期の試行、同条件の比較動画は[全動画の履歴](docs/video-history.md)に残しています。
...
直近21回は、H01〜H04とL00〜L16の有限な試験を数えたものです。同じ学習のcheckpoint違い、追加動画、次回準備は加算しません。L07の15条件も1回の再設計検証にまとめています。

各回の変更・日時・測定値・次の判断は[21回の実行・評価履歴](docs/improvement-loops.md)、録画と比較条件は[全動画の履歴](docs/video-history.md)、集計用データは[履歴JSON](evidence/learning-history.json)を参照してください。
...
- [21回の機械可読な履歴](evidence/learning-history.json)

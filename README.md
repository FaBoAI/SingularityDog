...
**確認できた直近の改善系列は22回です。** H01〜H04とL00〜L17を数え、診断・失敗を含みます。成功22回や全開発の通算回数ではありません。

**第22回・L17は、左前足を振る挨拶を3条件それぞれ500更新し、評価まで完了しました。** 同じ親モデルから別々に学習した小・中・大を比較し、「大」は2往復と四足への復帰2秒保持を2周期とも達成しました。学習終了は09/11 07:14・08:01・08:49、追加評価を含む全処理は09:00 JSTに終了しています。

**総合判定は3条件とも未達です。** 「大」は最大位置ずれ7.76cm・方位ずれ5.83°が基準5cm・5°を超えました。「中」は4往復できましたが四足保持が1.985／1.980秒で僅かに不足、「小」は往復振幅が不足しました。各28秒の評価中、禁止接触・関節速度超過は0標本です。追加seedの実測軌道は同一で、異なる外乱に対する頑健性試験とは扱いません。[第22回の結果・課題](docs/l17-evaluation.md) · [3条件の同期比較・28秒](docs/media/l17-greeting-comparison.mp4) · [「大」の原動画](docs/media/l17-large-greeting.mp4)

第21回は伏せ・四足復帰の各2秒保持を1回達成し、ジャンプは未達でした。第18回では1000更新後、6方向が既存の12秒開発基準を通過し、前進40.2521cm/sを達成しています。課題・形状・条件が異なる過去の結果は[各回の履歴](docs/improvement-loops.md)に残しています。

**当面の新規学習は1試行500更新。** 第22回は第21回の形状・質量を維持し、最新指定の黄色い脚・取手と、黒を基調とした胴体・底板・固定具・顔枠を評価へ反映しました。カーボン・QDD・足先球は黒、IMUは赤、顔は黒地に白い目です。実機の組付け・PLA強度と、Fusionの最終色版アーカイブ保存は未完了です。[仕様の反映状況と次の課題](docs/next-loop-spec.md) · [L13/L14時点のPLA79個の重量表](docs/pla-parts-mass.md)。重量表は旧構成のCAD中実換算で、最新構成の実印刷重量ではありません。次は挨拶後に足を下ろす際の荷重移動と位置・向き保持を改善する候補です。新しい学習は未開始、後ろ足2本立ちは未学習です。

[第21回の伏せ・ジャンプ](docs/l16-evaluation.md) · [第20回の記録](docs/l15-evaluation.md) · [第19回の履歴](docs/l14-evaluation.md) · [第18回の6方向歩行](docs/l13-evaluation.md)

## 映像でたどる開発履歴

READMEでは**最初・よちよち歩き・第5・10・15・20回の動画ダイジェスト**を掲載します。全ての録画・比較動画27本とCAD画像1点は引き続き公開し、[全動画の履歴](docs/video-history.md)と[22回の実行・評価履歴](docs/improvement-loops.md)からたどれます。改善回数22回と動画本数27本は別の集計です。
...
|---|---|---|
| **最初**<br>旧形状・保存番号2197<br>直近22回より前 | <img src="docs/media/archive-joystick2197-left.gif" width="480" alt="最初の掲載動画・旧形状での左移動。保存番号2197は改善回数ではありません"><br>[元MP4・5.98秒](docs/media/archive-joystick2197-left.mp4) | 外装付きの旧モデルで左移動を試行。同じ方策の別評価では速度・姿勢条件を満たしたが、後の足別診断では15mm以上の完了足上げは全脚0回。すり足が課題。 |
...
「最初」は以前の一覧の先頭にあった旧形状の動画です。保存番号2197は改善回数ではなく、直近22回へ加算しません。「よちよち歩き」は既存の第4回H04で、新しい実験回数ではありません。第20回のプレビューは24秒全区間で、終端の自動リセットも含みます。間の回や初期の試行、同条件の比較動画は[全動画の履歴](docs/video-history.md)に残しています。
...
直近22回は、H01〜H04とL00〜L17の有限な試験を数えたものです。同じ学習のcheckpoint違い、追加動画、次回準備は加算しません。L07の15条件も1回の再設計検証にまとめています。

各回の変更・日時・測定値・次の判断は[22回の実行・評価履歴](docs/improvement-loops.md)、録画と比較条件は[全動画の履歴](docs/video-history.md)、集計用データは[履歴JSON](evidence/learning-history.json)を参照してください。
...
- [22回の機械可読な履歴](evidence/learning-history.json)

...
**第27回のA/B比較を2026-09-12 03:06:30 JSTに完了しました。** A・Bをそれぞれ500更新し、同じD17機体・親・seed・登録基準で9動作を評価。両条件とも親の3/9から**6/9動作が合格**しました。Aは前進**40.16cm/sを12秒間**達成。伏せの復帰と挨拶中の位置ずれが残課題です。

各条件の内訳は歩行300・伏せ120・挨拶80・お座り0更新。A/Bは別々の条件で、合計1000更新を1個の統合方策へ学習した結果ではありません。直近系列の27回はH01〜H04とL00〜L22で、失敗や診断も含む実行履歴です。
...
| 動作 | 条件A | 条件B |
|---|---|---|
| 前進 | 合格：40.16cm/s、12秒 | 未達：8.92秒で終了 |
| 後退 | 合格 | 合格 |
| 左移動 | 未達：方位15.43° | 合格 |
| 右移動 | 合格 | 合格 |
| 左旋回 | 合格 | 合格 |
| 右旋回 | 合格 | 合格 |
| 伏せ・四足復帰 | 未達 | 未達 |
| 高い挨拶・四足復帰 | 未達 | 未達：最大XY6.35cm |
| 浅いお座り・四足復帰 | 合格（親を保持） | 合格（親を保持） |

[第27回の結果・条件差・残課題](docs/l22-evaluation.md) · [選択値JSON](evidence/l22-evaluation.json) · [第26回の結果](docs/l21-evaluation.md) · [第25回の9動作](docs/l20-evaluation.md) · [現在地](docs/current-status.md)
...
READMEでは**最初・よちよち歩き・第5・10・15・20回の動画ダイジェスト**を掲載します。公開している録画・比較動画41本とCAD画像1点は引き続き保持し、[全動画の履歴](docs/video-history.md)と[27回の実行・評価履歴](docs/improvement-loops.md)からたどれます。改善回数27回と動画本数41本は別の集計です。
...
|---|---|---|
| **最初**<br>旧形状・保存番号2197<br>直近27回より前 | <img src="docs/media/archive-joystick2197-left.gif" width="480" alt="最初の掲載動画・旧形状での左移動。保存番号2197は改善回数ではありません"><br>[元MP4・5.98秒](docs/media/archive-joystick2197-left.mp4) | 外装付きの旧モデルで左移動を試行。同じ方策の別評価では速度・姿勢条件を満たしたが、後の足別診断では15mm以上の完了足上げは全脚0回。すり足が課題。 |
...
「最初」は以前の一覧の先頭にあった旧形状の動画です。保存番号2197は改善回数ではなく、直近27回へ加算しません。「よちよち歩き」は既存の第4回H04で、新しい実験回数ではありません。第20回のプレビューは24秒全区間で、終端の自動リセットも含みます。間の回や初期の試行、同条件の比較動画は[全動画の履歴](docs/video-history.md)に残しています。
...
直近27回は、H01〜H04とL00〜L22の有限な試験を数えたものです。同じ学習のcheckpoint違い、追加動画、次回準備は加算しません。L07の15条件も1回の再設計検証にまとめています。

各回の変更・日時・測定値・次の判断は[27回の実行・評価履歴](docs/improvement-loops.md)、録画と比較条件は[全動画の履歴](docs/video-history.md)、集計用データは[履歴JSON](evidence/learning-history.json)を参照してください。
...
- [27回の機械可読な履歴](evidence/learning-history.json)

...
**L05の実測結果：前16.54・後6.23・左8.05・右6.54cm/s。** 各方向の初期位置から12秒後までの世界座標変位を12秒で割り、2秒の始動も含めました。前進の15mm以上の足上げは14 / 14 / 1 / 13回。後進の後脚は両方0回、右移動では終端の向きが約49.7度ずれています。速度目標と歩行品質の両方が未達です。今回の実動画はローカルへ回収済みです。

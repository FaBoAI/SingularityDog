...
**第30回の学習と評価を2026-09-13 15:01:52 JSTに完了しました。** A・B・Cの各500更新、合計1,500新規更新を実施し、全条件で**7/9動作が合格**しました。移動6方向とお座りを維持し、伏せと挨拶の位置・向きが残課題です。

各条件の内訳は伏せ250・挨拶250・歩行0・お座り0更新。同じ選択親・seed42から独立に学習した4専門方策の結果で、単一方策の9技能習得や連続切替ではありません。API動作確認の計12更新は主更新に含めません。

| 第30回の動作 | 全区間の結果 |
|---|---|
| 前進・後退・左右移動・左右旋回 | A・B・Cすべて合格、各12秒。歩行モデルは追加学習せず保持 |
| 伏せ・四足復帰 | 全条件で総合未達。B・Cは伏せ保持と復帰を各2秒×2達成。Cでも最大位置ずれ57.52mm／基準50mm、方位ずれ21.38°／基準5° |
| 高い挨拶・四足復帰 | 全条件で総合未達。挙上150mm以上・4往復・復帰を維持したが、Cの最大位置ずれ63.02mm／基準50mm |
| 浅いお座り・四足復帰 | A・B・Cすべて合格、各32秒。親モデルを保持 |

B・Cは伏せの連続保持を改善しましたが、親が合格していた位置の基準を失いました。Cの挨拶では曝露hが下がる一方、足高さの余裕と終端位置ずれはBより悪化しています。合格数の維持や一部指標の改善を、全面的な改善とは扱いません。

全27最終評価と親を含む54評価を照合し、新しい保存モデル6本はCPU検査・手元保存を完了しました。元400Hz配列の独立再計算、複数seedの頑健性、実機検証は別工程です。

[第30回の結果・比較・残課題](docs/l25-evaluation.md) · [選択値JSON](evidence/l25-evaluation.json) · [第29回と親の選定](docs/l24-evaluation.md) · [現在地](docs/current-status.md)

## 最新の公開動画：第27回A/Bを3×3で比較

第28〜30回の動画はこの公開版では未掲載です。以下は第27回の評価映像です。
...
READMEでは**最初・よちよち歩き・第5・10・15・20・25回の動画ダイジェスト**を掲載します。公開している録画・比較動画41本とCAD画像1点は引き続き保持し、[全動画の履歴](docs/video-history.md)と[30回の実行・評価履歴](docs/improvement-loops.md)からたどれます。改善回数30回と動画本数41本は別の集計です。
...
|---|---|---|
| **最初**<br>旧形状・保存番号2197<br>直近30回より前 | <img src="docs/media/archive-joystick2197-left.gif" width="480" alt="最初の掲載動画・旧形状での左移動。保存番号2197は改善回数ではありません"><br>[元MP4・5.98秒](docs/media/archive-joystick2197-left.mp4) | 外装付きの旧モデルで左移動を試行。同じ方策の別評価では速度・姿勢条件を満たしたが、後の足別診断では15mm以上の完了足上げは全脚0回。すり足が課題。 |
...
「最初」は以前の一覧の先頭にあった旧形状の動画です。保存番号2197は改善回数ではなく、直近30回へ加算しません。「よちよち歩き」は既存の第4回H04で、新しい実験回数ではありません。第20回のプレビューは24秒全区間で、終端の自動リセットも含みます。間の回や初期の試行、同条件の比較動画は[全動画の履歴](docs/video-history.md)に残しています。
...
直近30回は、H01〜H04とL00〜L25の有限な試験を数えたものです。同じ学習のcheckpoint違い、追加動画、次回準備は加算しません。L07の15条件も1回の再設計検証にまとめています。

各回の変更・日時・測定値・次の判断は[30回の実行・評価履歴](docs/improvement-loops.md)、録画と比較条件は[全動画の履歴](docs/video-history.md)、集計用データは[履歴JSON](evidence/learning-history.json)を参照してください。
...
- [30回の機械可読な履歴](evidence/learning-history.json)

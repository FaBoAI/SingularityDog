...
**確認できた直近の改善系列は12回です。** 初期立位基準を除くH01〜H04とL00〜L07を数えています。診断・失敗も含み、成功が12回という意味ではありません。全開発の通算回数は未確定です。

DIY四足ロボットの設計・強化学習で得た、独自のノウハウと検証ツールを公開しています。**第12回・L07**では学習済み方策を固定して制御を再設計し、15ケースをGPU評価しました。前進は**18.64cm/s（変位/12秒）**へ改善。ただし20cm/sは未達で、後退・左で関節速度が仮上限を超えた足幅案は全方向共通の制御として採用していません。**追加学習は0更新**です。

[今回の結果・何を変えたか](docs/l07-evaluation.md) · [前進の足幅比較動画](docs/media/l07-forward-stance-comparison.mp4) · [接触で停止した比較動画](docs/media/l07-forward-stride-failures.mp4)

## 映像でたどる開発履歴

個別に公開を指定された**CAD画像1点と、実際のシミュレータ録画・比較動画10本**を掲載しています。元のMP4は全フレーム・50fpsのまま保存。下のGIFは全区間を等速で表示する縮小プレビューです。段階ごとに形状・物理設定・指令・制御方式が異なる履歴で、同条件の性能比較ではありません。
...
|---|---|---|
| 旧形状・方向応答の試行<br>保存番号2197<br>直近12回より前 | <img src="docs/media/archive-joystick2197-left.gif" width="320" alt="旧モデル・保存番号2197の左移動録画"><br>[元MP4・5.98秒](docs/media/archive-joystick2197-left.mp4) | 外装付きモデルで左移動を試行。当時の別評価では速度・姿勢条件を満たしたが、後の足別診断では15mm以上の完了足上げは全脚0回。すり足が課題。 |
| 形状・接触表現を更新<br>保存番号250<br>直近12回より前 | <img src="docs/media/archive-dev250-left.gif" width="320" alt="別モデル・保存番号250の左移動録画"><br>[元MP4・11.98秒](docs/media/archive-dev250-left.mp4) | 左へ5cm/sの指令。別の4環境評価では平均約0.16cm/s、全脚の完了足上げ0回。姿勢は保てても移動は未達。 |
...
旧2本の数値は同じ方策の**別評価**で、この録画から直接測った値ではありません。2197・250は保存番号で、改善ループの回数ではありません。H04・L00は既存12回に含まれ、B0・旧2本・CAD画像を今回の掲載だけで追加計上しません。ファイル名の「(1)」は元ファイルと同じ内容でした。[対応関係・日時・評価条件](docs/video-history.md)も記録しています。
...
四方向用の比較基準は「足幅追加なし＋35mm足上げ＋補正」です。**前17.46・後10.92・左9.94・右10.61cm/s**、各脚13〜14回の15mm足上げを確認しました。速度は起動2秒を含む初期位置から12秒後までの固定世界方向の変位/12秒です。

動画に示す20mm足幅案では前進18.64cm/sに達しましたが、後退と左で実関節速度が最大13.22rad/sとなり、仮上限10rad/sを超えました。完走・トルク制限内というだけで歩行合格にしません。新しい足パッドの形状は別に検査しましたが、この動画・速度は従来形状の結果です。[15ケースの測定値と判断](docs/l07-evaluation.md)を参照してください。
...
- [12回の機械可読な履歴](evidence/learning-history.json)

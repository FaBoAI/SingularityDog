...
**第32回を2026-09-14 15:11:45 JSTに完了しました。** A・Bの各500更新、合計1,000新規主更新を実施し、両条件で**7/9動作が合格、新たな合格動作は0**でした。移動6方向とお座りを維持し、伏せと挨拶は総合未達です。

各条件は伏せ250・挨拶250・歩行0・お座り0更新。伏せは第30回C、挨拶は第30回Aの同じ選択親・seed42から独立に学習しました。第31回Aの継続学習ではありません。4専門方策を個別に評価した結果で、単一方策の9技能習得や連続切替ではありません。API動作確認の計8更新は主更新と別枠です。

| 第32回の結果 | 維持・改善と残った課題 |
|---|---|
| 移動6方向・お座り | A/Bとも合格。追加学習0で保持 |
| Aの伏せ・100更新時点 | 24秒を完走し、姿勢・復帰保持を維持。未達は最大XY102.14mmのみで、50mm基準を超過 |
| 最終250更新の伏せ | Aは11.74/24秒、Bは9.94/24秒で禁止接触終了。更新数の増加に伴う退行を記録 |
| 挨拶 | 100・200・250更新の全6評価が36秒完走。未達は位置ずれで、最小のA250も62.078mm。親62.390mmから約0.311mmの変化に留まる |

100・200更新時点の保存モデルは、250更新の学習終了後に評価しました。中間評価の追加PPO更新は0です。A/B×伏せ・挨拶×100・200・250更新の**全12評価が総合未合格**で、単一seedの微小差を改善の確証とは扱いません。

![第32回の保存モデル比較。数値から作成した図で、動画ではありません。](docs/media/l27-checkpoint-comparison.png)

親・最終の36評価と中間8評価、最終4モデルのCPU検査・手元保存を確認しました。比較用12時系列の判定・件数は凍結した数値評価器で再計算し、元の集計と一致しました。中間8モデルの本体は遠隔側のSHA・サイズ確認までで、ローカルCPU再検証とは区別します。学習は固定D17で行い、本日の印刷部品補強は混ぜていません。

**第32回について公開済みなのは数値と比較図です。第32回の動画は未取得・未掲載で、最新の公開動画は下の第31回A/B/C、公開実録画・比較MP4は47本のままです。** 第32回の結果公開時点で、モデルの実機採用・次の追加学習は実施していません。

[第32回の結果・比較・残課題](docs/l27-evaluation.md) · [選択値JSON](evidence/l27-evaluation.json) · [第31回の結果と動画](docs/l26-evaluation.md) · [現在地](docs/current-status.md)
...
## 最新の印刷用STL：黒い共通底板と下段ポール固定

**JetsonとMakita 40Vの固定枠を一枚の底板へ統合しました。** K1 Max用の280×130×26mm、黒PLAです。Jetson側のねじ頭が当たる2か所を修正し、上押さえ4穴と下段φ20ポール用クランプ4組・8部品の固定経路を確認しました。

[全STL・組立図・ねじ一覧](docs/printing-common-deck-k1max.md) · [公開ZIP](docs/printing/common-deck-k1max-r2/FaBoRobotDog_Black_Deck_and_PoleClamps_K1Max_r2_public.zip)

![黒い共通底板と下段ポール用クランプの組立図。青いクランプは説明色、印刷指定は黒。](docs/printing/common-deck-k1max-r2/previews/common_plate_mounting_underside_exploded.png)

個別11部品とクランプの配置済み代替STLを収録。PLA中実換算は合計296.43gで、実消費量とは異なります。ねじ穴・静的干渉・閉じたSTLを確認済みですが、現物合わせ・荷重強度・スライスは未実施です。
...
READMEでは**最初・よちよち歩き・第5・10・15・20・25回の動画ダイジェスト**を掲載します。公開している録画・比較動画47本とCAD画像1点は引き続き保持し、[全動画の履歴](docs/video-history.md)と[32回の実行・評価履歴](docs/improvement-loops.md)からたどれます。改善回数32回と動画本数47本は別の集計です。
...
|---|---|---|
| **最初**<br>旧形状・保存番号2197<br>直近32回より前 | <img src="docs/media/archive-joystick2197-left.gif" width="480" alt="最初の掲載動画・旧形状での左移動。保存番号2197は改善回数ではありません"><br>[元MP4・5.98秒](docs/media/archive-joystick2197-left.mp4) | 外装付きの旧モデルで左移動を試行。同じ方策の別評価では速度・姿勢条件を満たしたが、後の足別診断では15mm以上の完了足上げは全脚0回。すり足が課題。 |
...
「最初」は以前の一覧の先頭にあった旧形状の動画です。保存番号2197は改善回数ではなく、直近32回へ加算しません。「よちよち歩き」は既存の第4回H04で、新しい実験回数ではありません。第20回のプレビューは24秒全区間で、終端の自動リセットも含みます。間の回や初期の試行、同条件の比較動画は[全動画の履歴](docs/video-history.md)に残しています。
...
直近32回は、H01〜H04とL00〜L27の有限な試験を数えたものです。同じ学習のcheckpoint違い、追加動画、次回準備は加算しません。L07の15条件も1回の再設計検証にまとめています。

各回の変更・日時・測定値・次の判断は[32回の実行・評価履歴](docs/improvement-loops.md)、録画と比較条件は[全動画の履歴](docs/video-history.md)、集計用データは[履歴JSON](evidence/learning-history.json)を参照してください。
...
- [32回の機械可読な履歴](evidence/learning-history.json)

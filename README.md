...
**確認できた直近の改善系列は17回です。** H01〜H04とL00〜L12を数え、診断・失敗を含みます。成功17回や全開発の通算回数ではありません。

第17回・L12は500更新・4,096,000遷移を完了。前進は11.14秒で非足接触により終了し、38.6958cm/sはその区間だけの平均です。40cm/sと12秒完走は未達。ほか5方向は12秒完走し、全開発基準を満たしたのは右移動の1/6方向です。 左移動の漂流・滑りと左旋回の位置保持は改善した一方、速度不足が残りました。単一seedのシミュレーション開発評価です。

[第17回の結果・課題](docs/l12-evaluation.md) · [6方向動画](docs/media/l12-six-directions.mp4) · [同条件のL11親比較動画](docs/media/l12-matched-l11-six-directions.mp4) · [第16回の履歴](docs/l11-evaluation.md)

## 映像でたどる開発履歴

個別に公開を指定された**CAD画像1点と、実際のシミュレータ録画・比較動画19本**を掲載しています。元のMP4は全フレーム・50fpsのまま保存。下のGIFは全区間を等速で表示する縮小プレビューです。段階ごとに形状・物理設定・指令・制御方式が異なる履歴で、同条件の性能比較ではありません。
...
|---|---|---|
| 旧形状・方向応答の試行<br>保存番号2197<br>直近17回より前 | <img src="docs/media/archive-joystick2197-left.gif" width="320" alt="旧モデル・保存番号2197の左移動録画"><br>[元MP4・5.98秒](docs/media/archive-joystick2197-left.mp4) | 外装付きモデルで左移動を試行。当時の別評価では速度・姿勢条件を満たしたが、後の足別診断では15mm以上の完了足上げは全脚0回。すり足が課題。 |
| 形状・接触表現を更新<br>保存番号250<br>直近17回より前 | <img src="docs/media/archive-dev250-left.gif" width="320" alt="別モデル・保存番号250の左移動録画"><br>[元MP4・11.98秒](docs/media/archive-dev250-left.mp4) | 左へ5cm/sの指令。別の4環境評価では平均約0.16cm/s、全脚の完了足上げ0回。姿勢は保てても移動は未達。 |
...
旧2本の数値は同じ方策の**別評価**で、この録画から直接測った値ではありません。2197・250は保存番号で、改善ループの回数ではありません。H04・L00は既存17回に含まれ、B0・旧2本・CAD画像を今回の掲載だけで追加計上しません。ファイル名の「(1)」は元ファイルと同じ内容でした。[対応関係・日時・評価条件](docs/video-history.md)も記録しています。
...
- [17回の機械可読な履歴](evidence/learning-history.json)

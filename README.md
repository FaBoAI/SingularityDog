...
**確認できた直近の改善系列は10回です。** 初期の立位基準は回数に含めず、H01〜H04とL00〜L05を数えています。診断や失敗も含む10回であり、成功が10回という意味ではありません。全開発の通算回数は未確定です。PPOの更新数やフォルダ数とも区別しています。

DIY四足ロボットの設計・強化学習で得た、独自のノウハウと検証ツールを公開しています。第10回・L05は2026-09-09 11:49 JSTに500更新、11:53に4方向の録画評価を完了しました。最新の前進平均は**16.54cm/s（12秒の変位から算出）**。目標20cm/s、後脚の十分な足上げ、方向保持は未達です。

## 映像でたどる開発履歴

今回指定された**CAD画像1点と、実際のシミュレータ録画6本**を掲載しています。元のMP4は全フレーム・50fpsのまま保存。下のGIFは全区間を等速で表示する縮小プレビューです。段階ごとに形状・物理設定・指令・制御方式が異なる履歴で、同条件の性能比較ではありません。
...
|---|---|---|
| 旧形状・方向応答の試行<br>保存番号2197<br>直近10回より前 | <img src="docs/media/archive-joystick2197-left.gif" width="320" alt="旧モデル・保存番号2197の左移動録画"><br>[元MP4・5.98秒](docs/media/archive-joystick2197-left.mp4) | 外装付きモデルで左移動を試行。当時の別評価では速度・姿勢条件を満たしたが、後の足別診断では15mm以上の完了足上げは全脚0回。すり足が課題。 |
| 形状・接触表現を更新<br>保存番号250<br>直近10回より前 | <img src="docs/media/archive-dev250-left.gif" width="320" alt="別モデル・保存番号250の左移動録画"><br>[元MP4・11.98秒](docs/media/archive-dev250-left.mp4) | 左へ5cm/sの指令。別の4環境評価では平均約0.16cm/s、全脚の完了足上げ0回。姿勢は保てても移動は未達。 |
...
旧2本の数値は同じ方策の**別評価**で、この録画から直接測った値ではありません。2197・250は保存番号で、改善ループの回数ではありません。H04・L00は既存10回に含まれ、B0・旧2本・CAD画像を今回の掲載だけで追加計上しません。ファイル名の「(1)」は元ファイルと同じ内容でした。[対応関係・日時・評価条件](docs/video-history.md)も記録しています。
...
**L05の実測結果：前16.54・後6.23・左8.05・右6.54cm/s。** 各方向の初期位置から12秒後までの世界座標変位を12秒で割り、2秒の始動も含めました。前進の15mm以上の足上げは14 / 14 / 1 / 13回。後進の後脚は両方0回、右移動では終端の向きが約49.7度ずれています。速度目標と歩行品質の両方が未達です。[今回の4方向動画](docs/media/l05-four-directions.mp4)も公開しました。

過去のL00前進10.37cm/sは元の速度記録の平均です。同じ平均方法でL05は16.57cm/sとなり、上記の変位基準16.54cm/sとは計算方法を区別します。物理設定と指令も変わっているため、変更1つの因果効果や実機性能の向上率とは扱いません。L05は500更新と全128,000回の駆動計算を完了しましたが、実機同定・長時間評価・故障の根絶は未確認です。
...
- [10回の機械可読な履歴](evidence/learning-history.json)

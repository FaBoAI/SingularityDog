...
DIY四足ロボットの設計・強化学習で得た、独自のノウハウと検証ツールを公開しています。2026-09-09時点で、評価済みの前進平均は**10.37cm/s**。目標20cm/s、後脚の十分な足上げ、方向保持は未達です。第10回に当たる**L05はGPU学習中**です（09/09 11:13 JST開始）。
...
**第10回・L05を実行中です。** 修正モデルで256並列・500更新、4,096,000追加サンプルを学習します。11:16 JST時点で24更新と新しい重みの保存を確認しました。学習の正常終了後に最新の重みで4方向を録画・評価する処理も待機中です。実行・評価済みは9回のままで、今回の速度と足上げはまだ未評価です。L04は初期の指令段階だけを通過しており、実機同定や故障の根絶も未確認です。

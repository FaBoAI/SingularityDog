...
**確認できた直近の改善系列は11回です。** 初期の立位基準は回数に含めず、H01〜H04とL00〜L06を数えています。診断や失敗も含む11回であり、成功が11回という意味ではありません。全開発の通算回数は未確定です。PPOの更新数やフォルダ数とも区別しています。

DIY四足ロボットの設計・強化学習で得た、独自のノウハウと検証ツールを公開しています。**第11回・L06**は2026-09-09 13:01 JSTに500更新、13:06に4方向の録画評価を完了しました。前進平均は**16.99cm/s（固定世界軸の変位/12秒）**。前回16.54cm/sから微増しましたが、20cm/sと全脚の十分な足上げは未達です。

## 映像でたどる開発履歴

今回指定された**CAD画像1点と、実際のシミュレータ録画7本**を掲載しています。元のMP4は全フレーム・50fpsのまま保存。下のGIFは全区間を等速で表示する縮小プレビューです。段階ごとに形状・物理設定・指令・制御方式が異なる履歴で、同条件の性能比較ではありません。
...
|---|---|---|
| 旧形状・方向応答の試行<br>保存番号2197<br>直近11回より前 | <img src="docs/media/archive-joystick2197-left.gif" width="320" alt="旧モデル・保存番号2197の左移動録画"><br>[元MP4・5.98秒](docs/media/archive-joystick2197-left.mp4) | 外装付きモデルで左移動を試行。当時の別評価では速度・姿勢条件を満たしたが、後の足別診断では15mm以上の完了足上げは全脚0回。すり足が課題。 |
| 形状・接触表現を更新<br>保存番号250<br>直近11回より前 | <img src="docs/media/archive-dev250-left.gif" width="320" alt="別モデル・保存番号250の左移動録画"><br>[元MP4・11.98秒](docs/media/archive-dev250-left.mp4) | 左へ5cm/sの指令。別の4環境評価では平均約0.16cm/s、全脚の完了足上げ0回。姿勢は保てても移動は未達。 |
...
旧2本の数値は同じ方策の**別評価**で、この録画から直接測った値ではありません。2197・250は保存番号で、改善ループの回数ではありません。H04・L00は既存11回に含まれ、B0・旧2本・CAD画像を今回の掲載だけで追加計上しません。ファイル名の「(1)」は元ファイルと同じ内容でした。[対応関係・日時・評価条件](docs/video-history.md)も記録しています。
...
**L06の実測結果：前16.99・後6.65・左7.06・右9.08cm/s。** 各方向の初期位置から12秒後までの固定世界軸の変位を12秒で割り、2秒の始動も含めています。[今回の4方向評価動画](docs/media/l06-four-directions.mp4)には実測平均と15mm以上の完了足上げ回数を表示しました。

前進の足上げは **14 / 14 / 0 / 12回**（前左 / 前右 / 後左 / 後右）。後退の後脚は両方0回です。右の終端の向きずれは約49.7度から18.4度へ縮小しましたが、左は約41.1度ずれています。前進速度の微増を歩行合格とは扱いません。

L06はL05と同じ形状・物理・指令・元の学習重みから、足上げ誤差と接地タイミングの報酬係数だけを4倍にした比較です。L05最終重みからの継続ではありません。単一seedの結果で、一般的な改善効果や実機性能は未確認です。元速度記録の平均では前17.01・後6.62・左7.02・右9.09cm/sとなり、変位基準の値とは区別しています。[測定結果と条件](evidence/l06-evaluation.json)も保存しています。
...
- [11回の機械可読な履歴](evidence/learning-history.json)

...
function view(key){const x=loops.find(l=>l.id===document.getElementById(key).value);let v=x.video_url?`<video controls preload="metadata" playsinline src="${esc(x.video_url)}"></video>`:`<p>${(x.video||x.video_available_privately)?'公開版には動画を同梱していません。':'このループの記録動画はありません。'}<br>${esc(labels[x.state])}</p>`;document.getElementById(key+'View').innerHTML=`<h3>${esc(x.id+' · '+x.title)}</h3><div class="video">${v}</div><p><b>変更：</b>${esc(x.change)}</p><p><b>結果：</b>${esc(x.result)}</p><p class="numbers">前進平均 ${speed(x.forward_speed_m_s)}</p><p class="note">${esc(x.comparison_scope)}</p>`;}
